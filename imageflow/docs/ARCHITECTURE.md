...
- Background execution also checks a quiet-server gate before touching queued work: normalized one-minute server usage must be below `background_max_load_percent` when `background_low_load_only=1`. The legacy raw-load value `background_max_load_1m` is still reported for diagnostics, and optional quiet hours can restrict processing to a server-local time window.
